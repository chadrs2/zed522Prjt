...
def visual_ISAM2_plot(result):
    ...
    plt.pause(1 / 30)  # 30 frames a second
    ...
    while True:
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            zed.retrieve_measure(
                point_cloud, sl.MEASURE.XYZRGBA
            ...
            # initial_xi = noisevisual_ISAM2_plot(current_estimate)ent (1.4m/s to millimeters) / frame rate
            # Camera is 30 fps
            x_dist = (1.4 * 1000) / 30
            x_dist = (
                -x_dist
            )  # we're walking left in respect to the frame, which is negative in Zed frame
            if i == 0:
                initial_xi = Pose3(Rot3(1, 0, 0, 0, 1, 0, 0, 0, 1), Point3(0, 0, 0))
            elif i == 1:
                initial_xi = Pose3(Rot3(1, 0, 0, 0, 1, 0, 0, 0, 1), Point3(0, 0, 0))
            ...
            if i == 0:
                ...
                for point in key_pts2:
                    ...
                    if not (
                        math.isnan(point3D[0])
                        or math.isnan(point3D[1])
                        or math.isnan(point3D[2])
                    ):
                ...
                pose0 = Pose3(Rot3(1, 0, 0, 0, 1, 0, 0, 0, 1), Point3(0, 0, 0))
                ...
                for point in key_pts2:
                    ...
                    if not (
                        ...
                        dist = math.sqrt(
                            point3D[0] ** 2 + point3D[1] ** 2 + point3D[2] ** 2
                        )
                        if dist < 10:
                            error = 0.005 * 1000
                        elif dist < 15:
                    ...
                    if not (l in curr_kp_dict):
                        ...
                        if not (
                            ...
                            if i != 1:
                                ...
                                cameraFramePoint = np.asarray(
                                    [point3D[0], point3D[1], point3D[2], 1]
                                )
                            ...
                            factor = PriorFactorPoint3(
                                L(curr_kp_dict[l]), init_lj, point_noise
                            )
                            graph.push_back(factor)
                # print(curr_kp_dict)
                ...
                for kk in range(1):  # how many extra times to iterate
                    isam.update()
